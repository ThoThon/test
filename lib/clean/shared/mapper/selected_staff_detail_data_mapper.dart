...
class SelectedStaffDetailDataMapper
  ...
  SelectedStaffDetail mapToEntity(SelectedStaffDetailData? data) {
    return SelectedStaffDetail(
      ...
      gender: data?.gioiTinh?.rawValue,
      onlyYearOfBirth: data?.chiCoNamSinh?.rawValue,
      ...
      baseSalaryCoefficient: int.tryParse(data?.heSoLuongCoBan ?? ''),
      salary: int.tryParse(data?.tienLuong ?? ''),
      positionAllowance: int.tryParse(data?.phuCapChucVu ?? ''),
      seniorityBeyondFrameAllowance:
          int.tryParse(data?.phuCapThamNienVuotKhung ?? ''),
      seniorityAllowance: int.tryParse(data?.phuCapThamNienNghe ?? ''),
      salaryAllowance: int.tryParse(data?.phuCapLuong ?? ''),
      additionalAllowance: int.tryParse(data?.phuCapBoSung ?? ''),
      ...
      contributionRate: data?.tyLeDong,
